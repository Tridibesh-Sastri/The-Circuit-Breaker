  ...
  useEffect(() => {
    ...
    const {
      ...
      if (event === "SIGNED_IN" && session) {
        ...
        checkAndCreateUserProfile(session.user.id).then(() => {
      ...
      if (!existingProfile) {
        ...
        if (user) {
          ...
          await supabase.from("profiles").insert({
            id: userId,
            username: user.email?.split("@")[0] || `user_${Math.random().toString(36).substring(2, 9)}`,
    ...
    try {
      ...
      if (data.user) {
        ...
        toast({
          ...
          description: "Your account has been created and your role request has been submitted for approval.",
        })
